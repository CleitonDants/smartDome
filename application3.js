window.addEventListener('DOMContentLoaded', () => {
    ...
    socket.on('esta ligado', function(msg){
        if(msg=="8on"){
            num = 1;
            document.getElementById('clique8').click();
        }
    })
	document.getElementById('clique8').addEventListener('click', () =>{
        if(num == 0){
            socket.emit('ligar', '8');
        }
        num = 0;
	...
	const erro = document.getElementById('clique8');
	let checa = 0;
		erro.addEventListener('click', () =>{
			if(checa==0){
				alert(`Lâmpada indisponível ou queimada.`);
				checa=1;
			}
			erro.click();
			erro.touchstart();
